...
function loadStoresData(indexStoreId) {
    $.get("data/store_addresses.csv", function(data) {
        ...
        if (storeArray[indexStoreId] == undefined) 
            {   if (strStoreId === undefined)
                    paramsError(CST_PARAMS_ERROR_INVALID_QUERY); 
                else if (strStoreId == "")
                    paramsError(CST_PARAMS_STORE_ID_NO_PARAMETER); // No parameter has been entered.
                else
                    paramsError(CST_PARAMS_ERROR_NO_STORE); // There is no store with this id. (id outside of the file)
                }
        else if (storeArray[indexStoreId][0] == "")
            paramsError(CST_PARAMS_ERROR_NO_STORE); // There is no store with this id. (blank value inside the csv file)
        else
        {
